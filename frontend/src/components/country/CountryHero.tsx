      ...
      <div className="relative">
        {country.heroImage?.url ? (
          ...
          <div className="flex h-64 w-full items-center justify-center bg-gradient-to-r from-[#27312d] to-[#151917] md:h-80">
            <p className="text-sm text-[#AEB9B1]">{dictionary.countryHero.noHeroImage}</p>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-[#AEB9B1]">
              {dictionary.countryHero.adventureCard}
            </p>
          ...
          <div className="min-w-[210px] rounded-2xl border border-[#D99E6B]/65 bg-[#D99E6B]/16 px-4 py-3 shadow-[0_0_28px_rgba(217,158,107,0.22)]">
            <p className="text-[10px] uppercase tracking-[0.2em] text-[#E7C7A1]">
              {dictionary.countryHero.minimalVsOptimal}
            </p>
            <p className="mt-1 text-2xl font-semibold tracking-tighter text-[#F6DEBE]">
              {country.minDays ?? dictionary.common.unknown} -{" "}
              {country.optimalDays ?? dictionary.common.unknown} {dictionary.countryHero.daysLabel}
            </p>
          </div>
        </div>

        <div className="flex flex-wrap gap-2">
          <RatingPill label={dictionary.countryHero.hiking} value={country.hikingLevel} />
          <RatingPill label={dictionary.countryHero.beach} value={country.beachLevel} />
          <RatingPill
            label={dictionary.countryHero.roadtrip}
            value={country.roadtripLevel}
            accent="roadtrip"
          />
        </div>
